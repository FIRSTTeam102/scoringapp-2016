...
<script type="text/javascript">
	...
	function drawArenaWithChanges() {
		for (var i = 0; i < defensePositions.length - 1; i++) {
			...
			if (defenseToCheck.position.x == dynamicDefense.position.x
					&& defenseToCheck.position.y == dynamicDefense.position.y) {
		...
		for (var i = 0; i < defensePositions.length; i++) {
			...
			ctx.drawImage(defenseToDraw.image, defenseToDraw.position.x,
					defenseToDraw.position.y, defenseToDraw.position.width,
					defenseToDraw.position.height);
		}
		...
		if (teamAsSpyBot != "") {
			if (side == "Near") {
				...
				ctx.fillStyle = "white";
				ctx.fillText("Team " + teamAsSpyBot, 341 + 134 / 2,
						127 + 102 / 2 + 7, 134);
			} else {
				ctx.font = "24px serif";
				ctx.textAlign = "center";
				ctx.fillStyle = "white";
				ctx.fillText("Team " + teamAsSpyBot, 99 + 134 / 2,
						472 + 102 / 2 + 7, 134);
			}
		}
		...
		for (var i = 0; i < robotPositions.length - 1; i++) {
			...
			if (robotToCheck.position.x == dynamicRobot.position.x
					&& robotToCheck.position.y == dynamicRobot.position.y) {
		...
		for (var i = 0; i < robotPositions.length; i++) {
			...
			if (team1Div.innerHTML.includes(robotToCheck.teamNumber)
					&& document.getElementById("team1Checkbox").checked
					|| team2Div.innerHTML.includes(robotToCheck.teamNumber)
					&& document.getElementById("team2Checkbox").checked
					|| team3Div.innerHTML.includes(robotToCheck.teamNumber)
					&& document.getElementById("team3Checkbox").checked) {
				robotPositions.splice(i, 1);
			}
		}
		for (var i = 0; i < robotPositions.length; i++) {
			var robotToDraw = robotPositions[i];
			ctx.fillText("Team " + robotToDraw.teamNumber,
					robotToDraw.position.x + 77 / 2,
					robotToDraw.position.y + 58 / 2 + 3,
					robotToDraw.position.width);
		}
	}
